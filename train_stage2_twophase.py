...
import yaml
import shutil
from dataset.twophase_flow_stage2 import TankSloshingData
...
class DilatedResidualBlock(nn.Module):
    def __init__(self, dim, dilation=1, padding_mode='circular'):
        ...
        self.conv = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(self.dim, self.dim, kernel_size=3, stride=1,
                      padding=(self.dilation, self.dilation), dilation=(self.dilation, self.dilation),
                      padding_mode=self.padding_mode),
            nn.GELU(),
            nn.Conv2d(self.dim, self.dim, kernel_size=3, stride=1, padding=1,
                      padding_mode=self.padding_mode),
