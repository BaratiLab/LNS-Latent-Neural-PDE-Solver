...
class NS2DData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train_mode:
            ...
            z = z[:self.case_len:self.interval]  # [t, h*w]
            z_in = z[start_t:start_t + self.in_tw]  # [tin, h*w]
        else:
            start_t = 0
        ...
        if self.train_mode:
            ...
            z_out = z[output_t:output_t+self.out_tw]  # [t, h*w]
            # normalize the output
            y = self.normalize_data(y)
            y = torch.from_numpy(y).float()  # [t, c, h, w]
            return x, y, z_in, z_out
        else:
            output_t = start_t + self.in_tw
