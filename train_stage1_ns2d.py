...
import yaml
import shutil
from dataset.ns2d_fno_stage1 import NS2DData        #, TankSloshingData
from utils import dict2namespace
import wandb
from training_utils import relative_lp_loss, prepare_training, log_images, log_sequence
...
class TrainAE:
    ...
    def train(self, args):
        ...
        for epoch in range(args.epochs):
            with tqdm(range(len(train_dataloader))) as pbar:
                ...
                for i, (x_last, x, x_next) in zip(pbar, train_dataloader):
                    x_last = x_last.to(self.device)
                    x = x.to(self.device)
                    x_next = x_next.to(self.device)

                    #x_all = torch.stack((x_last, x, x_next), dim=1)  # [b, 3, c, h, w]
                    x_all = x   #rearrange(x_all, 'b t c h w -> (b t) c h w')
                    x_hat = self.autoencoder(x_all)
                    rec_loss = relative_lp_loss(x_hat, x_all, reduce_dim=(-1, -2), p=2, reduce_all=True)
                    ...
                    pbar.set_postfix(
                        AE_Loss=np.round(ae_loss.cpu().detach().numpy().item(), 3),
                        EPOCH=epoch,
                    )
    ...
    def validate_loop(self, args, epoch_num):
        ...
        print(f'Validation Reconstruction Loss: {recon_loss}')

        # log some prediction
        log_sequence(x_hat[:, ::5, 0], os.path.join(self.log_dir, "samples", f"sample_{epoch_num}.png"))
        log_sequence(x[:, ::5, 0], os.path.join(self.log_dir, "samples", f"gt_{epoch_num}.png"))
        ...
        err = recon_loss_all.mean(dim=0).squeeze().cpu().numpy()  # preserve time axis
        err_std = recon_loss_all.std(dim=0).squeeze().cpu().numpy()
